...
public final class PunchHole {
    ...
    public void getCircleRect(Rect outRect) {
        ...
        android.view.DisplayCutout cutout = display != null ? display.getCutout() : null;
        if (cutout == null) {
            outRect.setEmpty();
            return;
        }
        if (hasType() && hasPosition()) {
            ...
            int cameraMargin = getStatusBarCameraTopMargin(context);
            if (cameraMargin < 0) {
                outRect.setEmpty();
                return;
            }
    ...
    public int getStatusBarCameraTopMargin(Context context) {
        @SuppressLint("DiscouragedApi")
        int resourceId = context.getResources().getIdentifier("status_bar_camera_top_margin", "dimen", "android");
